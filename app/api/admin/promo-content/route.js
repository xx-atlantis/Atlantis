...
const PAGE_SLUG = "promotions";
const SECTION_KEY = "promo-page";
const SECTION_TYPE = "JSON";

async function ensurePageSection() {
  let page = await prisma.page.findUnique({ where: { slug: PAGE_SLUG } });
  if (!page) {
    page = await prisma.page.create({ data: { slug: PAGE_SLUG, name: "Promotions" } });
  }

  let section = await prisma.section.findUnique({ where: { key: SECTION_KEY } });
  if (!section) {
    section = await prisma.section.create({ data: { key: SECTION_KEY, type: SECTION_TYPE } });
  }
...
export async function GET() {
  try {
    ...
    const result = { en: {}, ar: {} };
    section.translations.forEach((t) => { result[t.locale] = t.content; });
...
export async function PATCH(req) {
  try {
    ...
    console.error("Promo Content PATCH error:", err);
    return NextResponse.json({ success: false, error: err.message }, { status: 500 });
  }
}
