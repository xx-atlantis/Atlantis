...
export default function CheckoutPage() {
  ...
  const vat = subtotal * 0.05; // 5% VAT example (Check if you need 15% for KSA)
          ...
          {cart.cartType === "package" ? (
            ...
            <div className="border border-gray-200 rounded-xl p-5 md:p-8 shadow-sm bg-white">
              ...
              <div className="flex justify-between mb-4">
                <span className="font-semibold">{cart.package.title}</span>
                <span className="font-bold">{cart.package.price}</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                {Object.keys(cart.steps).map((key, index) => {
                  const step = cart.steps[key];
                  const title =
                    step.cardTitle ?? step.label ?? step.value ?? key;
