...
export default function CheckoutPage() {
  ...
  // SAFELY CAPTURE THE CUSTOMER ID REGARDLESS OF DATABASE FORMAT (id, _id, or uid)
  const customerId = customer?.id || customer?._id || customer?.uid || null;
    ...
    try {
      const res = await fetch("/api/admin/coupon/apply", {
        ...
        body: JSON.stringify({
          ...
          customerId: customerId, // <-- Updated to use safe ID
        }),
      });
  ...
  const handleCheckout = async () => {
    ...
    // <-- Updated to check the safe ID variable
    if (!customerId) {
        ...
        <div className="max-w-6xl mx-4 md:mx-auto grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-8">
          ...
          <div className="space-y-8">
            ...
            <div className="border border-gray-200 bg-white rounded-xl p-6 md:p-8 shadow-sm">
              ...
              {/* <-- Updated button to use the safe customerId check --> */}
              <button
              ...
              {!customerId && <p className="text-[10px] text-center mt-2 text-red-400">{isRTL ? "يرجى تسجيل الدخول للمتابعة" : "Please login to proceed"}</p>}
            </div>
          </div>
        </div>
      </section>
