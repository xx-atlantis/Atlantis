...
export default function AdminHomePage() {
  const { data, loading } = useAdminContent("terms-and-condition");
  ...
  const saveSection = async (key, locale, content) => {
    ...
    const savePromise = fetch("/api/admin/content", {
      method: "PATCH",
      body: JSON.stringify({ page: "terms-and-condition", key, locale, content }),
    }).then(async (res) => {
    ...
    toast.promise(savePromise, {
      ...
      success: {
        render() {
          return (
            <div>
              <div className="font-bold text-sm">Legal Content Updated!</div>
              <div className="text-xs opacity-80">
                {key.toUpperCase()} ({locale.toUpperCase()}) is now live.
              </div>
            </div>
          );
        },
      ...
      error: {
        render: `❌ Error updating terms: ${key}`,
      }
    });
  };
  ...
  return (
    <main className="p-6">
      {/* 4. The Toast Container - Dark theme for a professional feel */}
      <ToastContainer 
        position="top-right"
        autoClose={3000}
        theme="light" 
      ...
      <h1 className="text-2xl font-bold mb-8 text-slate-800">Terms and Condition CMS</h1>

      {/* Render sections: content, hero, etc. */}
