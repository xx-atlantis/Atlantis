...
/* -------------------------------------------
   MAIN COMPONENT
-------------------------------------------- */
export default function OurProjectsAdmin() {
        ...
        >
          ...
          {data.list.map((proj, i) => (
            <div key={i} className="border rounded-lg p-5 mb-8 bg-gray-50 shadow-sm">
              ...
              <label>{t.reviewImage}</label> 
              <div className="relative w-18 h-18 border rounded-full overflow-hidden shadow text-xs">
                    <Image
                      src={images[i]?.reviewImage || "/placeholder.png"}
                      alt="reviewImage"
                      fill
                      className="object-cover"
                    />
                    <button
                      onClick={() => openImageModal(i, "reviewImage")}
                      className="absolute bottom-2 right-1 bg-black/70 text-white p-1 rounded-full text-sm flex items-center gap-1"
                    >
                      <Pencil size={14} />
                    </button>
                  </div>
