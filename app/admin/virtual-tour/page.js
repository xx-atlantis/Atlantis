...
export default function AdminHomePage() {
  const { data, loading } = useAdminContent("virtual-tour");
  ...
  const saveSection = async (key, locale, content) => {
    ...
    const savePromise = fetch("/api/admin/content", {
      method: "PATCH",
      body: JSON.stringify({ page: "virtual-tour", key, locale, content }),
    }).then(async (res) => {
    ...
    toast.promise(savePromise, {
      ...
      success: {
        render() {
          return (
            <div>
              <div className="font-bold text-sm">Tour Content Updated!</div>
  ...
  return (
    <main className="p-6">
      ...
      <h1 className="text-2xl font-bold mb-8 text-slate-800">Virtual Tour CMS</h1>
